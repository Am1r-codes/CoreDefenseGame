...
class MenuScreen(GameScreen):
    ...
    def __init__(self, game) -> None:
        ...
        self._subtitle_font = pygame.font.SysFont(None, 40)
    ...
    def draw(self) -> None:
        ...
        # titel
        title = self._title_font.render("Core Collapse", True, (0, 220, 255))
        title_rect = title.get_rect(center=(width // 2, height // 4))
        screen.blit(title, title_rect)

        # ondertitel
        subtitle = self._subtitle_font.render("Defend the AI Core", True, (180, 180, 200))
        sub_rect = subtitle.get_rect(center=(width // 2, height // 4 + 60))
        screen.blit(subtitle, sub_rect)

        # knipperende "Press ENTER" tekst (wissel elke 30 frames)
        if (self._frame_counter // 30) % 2 == 0:
